...
def classify_stage(entries: list[FileRecord], params: dict, ctx: dict) -> list[FileRecord]:
    ...
    - content：抽取文本摘要，按内容关键词规则（ctx.rules.content_rules）
      —— 无内置规则，必须由调用方（模型）根据目录实际内容提取关键词后提供
    ...
    if by == "type":
        ...
        if not content_rules:
            raise StageError(
                "classify(by=content) 需要内容分类规则，而本工具不内置任何规则。"
                "请让模型根据目录实际内容提取关键词生成规则文件，再用 --rules 传入；"
                "或执行 `file_organizer discover-rules --path <目录> --output rules.json` 自动提取。"
            )
        default = rules.get("default_folder", DEFAULT_FOLDER)
        for e in entries:
            if matches(e, where):
