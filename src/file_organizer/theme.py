...
def sample_files(
    ...
    *,
    ...
    per_file_chars 缺省等于 max_chars；设为更大值可让每个样本携带更多上下文。
    """
    root = str(Path(path).expanduser().resolve())
    result = scan_directory(root)
    limit = per_file_chars if per_file_chars is not None else max_chars
    samples: list[tuple[str, str]] = []
    for e in result.entries:
        text = extract_text(e.path, max_chars=limit).strip()
        if text:
...
def discover_content_rules(
    ...
    *,
    ...
    samples = sample_files(
        path, max_files=max_files, max_chars=max_chars, per_file_chars=per_file_chars
    )
    if not samples:
        return {}
    if llm is None:
