...
def _add_discover_rules_parser(subparsers) -> None:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="每个样本实际携带的字符数（默认等于 --max-chars）",
    )
    p.add_argument(
        "--default-folder",
        default=None,
        help="未命中任何主题时的兜底目录（写入规则文件，默认 其他）",
    )
...
def _cmd_discover_rules(args) -> int:
    try:
        ...
        print(f"错误: {exc}", file=sys.stderr)
        return 2
    # 显式写出 default_folder，避免 load_rules 回退到内置默认规则
    full: dict = {"content_rules": content_rules}
    if args.default_folder:
        full["default_folder"] = args.default_folder
