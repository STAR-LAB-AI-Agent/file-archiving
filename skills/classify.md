...
### 示例 3
- 用户：「按内容分类」
- **注意**：`by=content` **不内置任何关键词规则**，未提供规则时会直接报错。必须先根据目录实际内容
  提取关键词生成规则文件，再通过 `--rules` 传入：
  ```bash
  file_organizer discover-rules --path <path> --output rules.json   # 或由模型自行提取
  file_organizer ask "按内容分类" --path <path> --rules rules.json --preserve-structure --yes
  ```
- 图片、音频、扫描件无正文，会落入 `其他/`，属预期行为。
