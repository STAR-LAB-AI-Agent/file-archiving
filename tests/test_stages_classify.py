...
def test_classify_by_content_without_rules_raises(make_record):
    """无内置内容规则：未提供规则时应报错，而不是用写死的规则瞎猜。"""
    entries = [make_record("a.txt")]
    with pytest.raises(StageError):
        classify_stage(entries, {"by": "content"}, {"rules": {}})
